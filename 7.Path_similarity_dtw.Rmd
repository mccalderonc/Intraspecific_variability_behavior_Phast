...
# compute DTW distance for each pair of individuals
  dtw_distances <- mapply(function(ind1, ind2) {
    dtw_result <- dtw(indiv_splits[[ind1]][, c("location_lat", "location_long")],
                      indiv_splits[[ind2]][, c("location_lat", "location_long")],
                      keep.internals = TRUE)
...
# gamma is the best distribution
...
# gamma model
mod_bw <- glmer(Distance~samegroup + (1|dyad), family = Gamma("log"), data=all_results)
...
new_data$predicted_distance <- predict(mod_bw, new_data, type = "response")
...
Independent of the group membership, the DTW index of commuting paths within and between groups did not differ. 
...
register_stadiamaps("useownkey") 

# create a bounding box
e <- make_bbox(location_long, location_lat, data = feb, f = .4)
...
ggsave(file="~/ownCloud/PhDLife/P.hastatus/Thesis/Paper2/PlosOne/figures/Fig3.pdf", width=15, height=10)
```
